...
- **Source:** Scryfall's *Oracle Cards* bulk file (~30–35k cards, one object per `oracle_id`), trimmed to a small set of gameplay fields plus a trimmed `card_faces` array for multi-face cards. Exact columns and rationale: [data-model.md](data-model.md). Target size well under 20 MB in Postgres.
- **Import filter:** the bulk file is not pre-filtered to gameplay cards. Rows are kept only if `games` includes `paper` **and** `layout` is not a non-gameplay layout (`art_series`, `token`, `emblem`, …) — the paper check alone lets art cards through. Full rule in [data-model.md](data-model.md).
- **Ingestion:** a server-side script (local `npm` script first, later a scheduled GitHub Action) downloads and stream-parses the file, filters, trims it, and upserts into the Supabase `cards` table using the `service_role` key. Never runs in the browser.
