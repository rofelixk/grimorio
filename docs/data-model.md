...
one object per `oracle_id`), refreshed ~weekly. Import filter: only cards whose `games`
array includes `paper`.
...
| `mana_cost` | `mana_cost` | Text. Empty for some multi-face layouts; per-face costs not captured in v1. |
...
| `layout` | `layout` | `normal`, `transform`, `modal_dfc`, `split`, `adventure`, … Signals whether face data exists. |
| `commander_legality` | `legalities.commander` | Text: `legal` / `not_legal` / `banned`. Only this one format is stored. |
| `image_url` | `image_uris.normal` | Text. Absent at top level for double-faced cards (see limitations). |
...
| `set`, `set_name`, `set_type`, `collector_number`, `rarity`, `artist`, `released_at` | Printing-level. Belong on a collection item once printing tracking exists. |
...
- **Multi-face cards** (`transform`, `modal_dfc`, `split`, `adventure`): only top-level
  fields are stored, so back faces, per-face mana costs/text, and back-face images are
  missing. These cards display partially. Fix = a trimmed `card_faces` structure. Deferred.
...
- **`card_faces`** for multi-face card display.
