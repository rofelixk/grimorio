...
export async function runCardOcr(blob: Blob): Promise<CardOcrResult> {
  try {
    ...
    try {
      ...
      return guessSetAndCollector(text);
    } finally {
    ...
    return {
      ...
      error:
        err instanceof Error
          ? `Não foi possível ler a carta automaticamente (${err.message}).`
          : 'Não foi possível ler a carta automaticamente.',
    };
  }
}
