...
- [ ] T027 [P] Create `src/app/core/services/profile-store.service.spec.ts`: case-insensitive/trimmed uniqueness, hash verification (small iterations via the token), `profiles()` never exposes `password`, persistence across a fresh instance (depends on T026)
...
- [ ] T049 [US1] In `entry-flow.store.ts` implement the `profile` submit: `validate()` (T013) → `ProfileStore.create({ name, password, colors: picks })` → `ProfileSessionService.activate(id)` → `done('profiled')`; busy label "Criando perfil…"; works offline (FR-021) (depends on T048)
- [ ] T050 [US1] Wire the desktop identity pane to show the `IdentityWheel` in picker mode during `profile` and retint the whole modal live to the picks (US1-3); picks start as the default identity `['R','U','G']` (`DEFAULT_IDENTITY`), then follow the picker rules (≤ 3, at least one always picked)
...
- [ ] T054 [P] [US1] Create `src/app/core/guards/profile.guard.spec.ts` covering the four outcomes (active → true; closed after a profile became active → true; closed without one → false after a prior navigation, `UrlTree('/')` on the initial one; a session-change reload with no active profile → `UrlTree('/')` without calling `open()`), with `EntryModalService` stubbed via a TestBed provider
...
- [ ] T080 [US4] Store: `setup` → `validate()` (T013; name + "Senha deste aparelho") → `setupFromPending` → `done('setup')` "Perfil pronto" with sync line "Baixando sua coleção…" → "Coleção baixada"; tinted with the account's colors (US4-5)
...
- [ ] T097 [P] 44×44 target audit across top bar, modal controls, links and swatches (FR-040, SC-009); the temporary profile button/menu is exempt (FR-029), though T060 still gives it 44px targets
- [ ] T098 [P] Copy audit: every visible string in the modal and top bar comes from `entry-copy.ts` and matches `STATES.md` verbatim; no raw backend text reachable; every modal `<form>` has `novalidate` and no native validation bubble can appear (try `type="email"`/`maxlength`/`required` inputs with bad values) (SC-006, Principle II)
