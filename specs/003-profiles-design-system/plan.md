...
| V. Zoneless, Signal-Driven Angular + DESIGN.md | The code is standalone/OnPush/signals with `linkedSignal`/`computed`/`effect`. There is no UI framework, and the modal uses native `<dialog>` and `popover`. New UI follows DESIGN.md. **Gap**: DESIGN.md has no app-shell top bar, so an "App top bar" entry is added to DESIGN.md before it is built (R16). **Deviation**: legacy `src/styles/*` global rules are removed all at once instead of "view by view" — see Complexity Tracking. | PASS with a justified deviation |
